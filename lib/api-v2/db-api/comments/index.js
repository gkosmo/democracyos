...
/**
 * Get the private listing of comments of an author from a topic
 * @method list
 * @param  {object} opts
 * @param  {objectId} opts.topicId
 * @param  {number} opts.limit - Amount of results per page
 ...
 * @return {promise}
 */
exports.userList = function list (opts) {
  opts = opts || {}

  return find({author: opts.user.id})
    .where({reference: opts.topicId})
    .populate(scopes.ordinary.populate)
    .select(scopes.ordinary.select)
    .limit(opts.limit)
 ...
 * @return {promise}
 */
exports.userListCount = function listCount (opts) {
  opts = opts || {}
